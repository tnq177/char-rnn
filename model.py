	...
	def __init__(self, config):
		...
		with tf.variable_scope(MODEL_SCOPE):
			...
			with tf.device(DEVICE_SCOPE):
				softmax_w 	= tf.get_variable("softmax_w", [config.batch_size, config.vocab_size])
				softmax_b 	= tf.get_variable("softmax_b", [config.vocab_size])
				embeddings 	= tf.get_variable("embeddings", [config.vocab_size, config.rnn_size])

				# After embedding lookup, inputs has shape (batch_size, seq_length, rnn_size)
				inputs 		= tf.nn.embedding_lookup(embeddings, self.input_data)
				# Split it into list of step input, i.e. along dimension 1
				inputs 		= tf.split(1, config.seq_length, inputs)
				'''
			...
			outputs, last_state = seq2seq.rnn_decoder(inputs, self.initial_state, cell, loop_function=loop if config.infer else None, scope=MODEL_SCOPE)
			# Concat each sequence of the batch 
			output = tf.concat(1, outputs) # now (batch_size x (seq_length x rnn_size))
			output = tf.reshape(output, [-1, rnn_size]) # now (batch_size x seq_length) x rnn_size 
